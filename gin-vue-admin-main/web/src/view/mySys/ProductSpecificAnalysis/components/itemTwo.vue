...
import { getGoodsProfitPublic } from '@/api/mySys/order';
...
async function fetchData() {
  try {
    const response = await getGoodsProfitPublic();
...
const renderChart = () => {
  ...
  const seriesData = data.value.map(item => ({ value: item.allProfit, name: item.goodsName })); // 商品类型及其数量作为饼状图数据
